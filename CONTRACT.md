...
| **Text tokens are set by contrast, not by mood** — `--body` #f7f7f5 (18:1), `--muted` #c6c4be (11.4:1), `--gold-dim` #cba54c (8.6:1); the marble ramp is a surface ramp and never a text colour | 2026-08-09 | Client read the site as washed-out and hard to read. The old warm greys (#e9e7e2 / #9a988f, and #81817f borrowed off the marble ramp for notes) measured 16:1 / 6.9:1 / 5.1:1 — technically AA, visibly dim on a phone in daylight, which is the actual viewing condition here. The gap between `--body` and `--muted` still carries the hierarchy; it is now a step in brightness rather than a drop into grey. |
| **The studios get real pages — `/fr/appartements` + one per unit — running on a fenced preview dataset** | 2026-08-09 | Client supplied the eleven unit codes and asked to see the pages working before the real content exists. The invention is contained rather than sprinkled: one `PREVIEW` flag in `lib/apartments.ts`, a notice on every page while it is true, `noindex` on the detail pages, and **no prices at all** — a made-up nightly rate is the one demo value a visitor could hold the venue to, and FACTS.md still lists rates as unknown. Codes are recorded in FACTS.md as client-sourced; availability, amenities and floors are marked ⚠️ preview in the data file. |
| **Reservation stays a WhatsApp/phone handoff — dates and headcount, prefilled** | 2026-08-09 | Keeps the "not a booking engine" decision above intact while giving the client the flow they asked to preview. The panel collects arrival, departure and guests and hands them to the channel the venue actually answers, with the unit code already in the message. It is real on the day it ships, so there is no fake confirmation screen to tear out when a true booking engine arrives — that panel is where it would go. |
| **Placeholder photography is a manifest state, not a convention** | 2026-08-09 | The seven studio images are a different building. `origin: "placeholder"` in `lib/photos.ts` keeps them out of `shippable()` (so no venue gallery can pick them up by accident) and obliges the « image d'illustration » caption wherever they do render. A page that wants one has to name it — which is the same page that owes the viewer the caption. |
...
| **`NEXT_DIST_DIR` env override for `distDir`** | Verification builds go to `.next-verify` and serve on 3201, so they can never again swap `.next` out from under the dev server on 3200. |
...
## Mobile round 2 — client feedback (2026-08-08)

Four reports from the phone: the banner does not respond to swipes, « Le lieu »
looks stretched, the footer is clumsy, and text is hard to read. All four
reproduced under measurement before anything changed.

| Finding | Measured | Decision |
| --- | --- | --- |
| **Banner had no swipe at all** — dots and arrows were the only controls | a scripted touch drag left the slide index unchanged | Pointer-event swipe on the section (touch, pen and mouse from one path). Axis is decided in the first 8px and a gesture that starts vertical is abandoned, so a scroll that drifts sideways cannot steal a slide; `touch-action: pan-y` keeps the page's own axis. A press that lands on a link or button is never swallowed. Threshold 45px. Swiping counts as taking over, so autoplay stops — same contract as the arrows. |
| **« Le lieu » cards were crushed, not stretched** | every card resolved to 335×456 (ratio 0.74) while five of the six photographs are 16:9; `object-fit: cover` was throwing away ~58% of each frame | The mobile band is **width-led**, the desktop band stays **height-led**. The old rule kept the desktop `height: 54vh` and then clamped `max-width: 86vw`, which silently overrode each photo's own ratio. Mobile is a one-up snap scroller, so *width* is the axis that must be consistent; 4:3 is the house frame ratio (Split, EventsIndex) so the band now matches the rest of the page. |
| The photo ratio could not be overridden by a media query | it was an inline `aspect-ratio` | It travels as `--card-ar` instead, and only the ≥900px rule consumes it. |
| **The footer was a wall of `.label`** | 14 consecutive lines of 11px uppercase mono at 0.19em; the footer stood 945px tall on an 844px screen | `.label` is an eyebrow treatment and was carrying the address, the number, the hours and the handles. Column *headings* stay `.label` — that is what gives the columns hierarchy; *values* are 15px sentence case. The number is display type, because it is the footer's one call to action. Same change to the Contact section's hours, which had the identical pattern. |
| `--sect` was used three times **inside** the footer | top padding + two margins, 80px each at 390px | `--sect` is the rhythm *between* sections; a component that uses it internally reads as three sections. The footer has its own, smaller internal step. |
| **The hero's caption parallax was not width-gated** | copy at 0.78 opacity after 300px of scroll, 0.67 after 450px, while still the only thing on screen | Gated to ≥900px. On desktop the caption is a glass panel over a full-viewport photograph on its way out, so fading it is the point; on the stacked mobile composition it just dims body copy. It also weakened the panel's own `backdrop-filter: brightness()` as it went, so the 13:1 measured at rest did not survive the scroll. |
| **Slide controls were below the fold on every phone** | 174px below at 320px, 106px at 360px, 70px at 390px, with 133–186px of dead space above them | The shared caption cell is as tall as the tallest caption and must stay that way, or the page jumps under the reader every 7 seconds. So the *bar* moves instead: `order: -1` below 900px puts the controls directly under the photograph, where a pagination row belongs, and moves the slack to the foot of the section where it reads as section spacing. |
| **The drawer's close button was painted permanently over the brand** | `opacity: 1`, `:focus-visible = false` — both wordmarks illegible | It carried `data-drawer-item`, and the stagger writes inline `opacity: 1; transform: none`, which beats the class rule that hides it until focus. Removed from the stagger; it has no entrance because it has no resting state. Its `top` also now clears the bar instead of sitting inside it. |
| **« Le lieu »'s progress hairline and counter were dead on mobile** | frozen at `0px` and « 01 / 06 » on every phone while all six cards scrolled past — visible in the client's own screenshot, card 06 on screen under a « 01 / 06 » counter | They were driven only from the pinned ScrollTrigger's `onUpdate`, which never runs below 900px. A `scroll` listener now feeds the same formula from `scrollLeft` wherever the pin is not active — the exact complement of the pin's query, so it also covers ≥900px under reduced motion, where they were equally dead. |

## Desktop round 2 — the three that were costing us (2026-08-09)

Asked what would keep the site out of an awards submission. The motion craft and
art direction are the strongest part of the build; what held it back was
mechanical, and all three are fixed here. The fourth — the « site en
construction » notice and the open `FACTS.md` questions — is a client decision,
not ours to close.

| Finding | Measured | Decision |
| --- | --- | --- |
| **« Le lieu » — the signature section — did not fit the fold while pinned** | overflowed by 28px at 1024×768, 83px at 1280×800, 93px at 1366×768, 60px at 1440×900, 102px at 1440×700; only 1920×1080 fitted | Two causes. The reserve was a flat `21rem`, but the section's `padding-block: var(--sect)` scales with viewport *width* (225px at 1024, 317px at 1440), so the estimate drifted further out the wider the window. And `--sect` — the between-sections rhythm — is far too much inside a section already claiming the whole screen. The reserve now subtracts the real padding, and the pinned padding is its own smaller token. Result: fits at every size from 1024×768 to 2560×1440 with 29–222px spare, and the photographs are *larger* than before (866×487 at 1440×900, against 928×522 that did not fit). |
| Cutting that padding pulled the eyebrow under the fixed header | « 04 — LE LIEU » sat beneath the nav once the section pinned at viewport 0 | The padding is asymmetric, because the two edges have different jobs: the top has a floor that clears the bar (~85px), the bottom does not need one. Verified: clears by 10–22px at every width. |
| **The nav's desktop mode engaged ~250px too early** | CTA right edge at 1025 of 900, 1036 of 960, 1050 of 1024 — 125px, 76px and 26px off-screen, hidden by `body { overflow-x: hidden }` | Link row moves from 900 → **1200**, the measured floor (1180) plus air. The drawer therefore covers everything below 1200 — wider than a usual burger range, but a working burger beats an invisible call to action. `Nav.tsx`'s resize-close guard carries the same number and must stay in step, or the page is left scroll-locked behind an invisible overlay. |
